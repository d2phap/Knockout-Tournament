...
class TournamentManager {
  ...
  createTournament(numberOfTeams, options = {}) {
    const teamsPerMatch = config.TEAMS_PER_MATCH;
...
class Tournament {
  ...
  static create(numberOfTeams, teamsPerMatch, options) {
    ...
    const numberOfRounds = Math.log(numberOfTeams) / Math.log(teamsPerMatch);

    if (numberOfRounds != Math.floor(numberOfRounds) || numberOfRounds <= 0) {
